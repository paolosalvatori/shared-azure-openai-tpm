...
The `kube-prometheus-stack-custom-values.yml` contains the definition of additional scrape jobs. In particular, the `kubernetes-pods` job scrapes metrics from pods with the `prometheus.io/scrape` set to `true`.
